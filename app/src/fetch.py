...
def fetch_wallet_data(address):
    ...
    result = {
        ...
        "network": "polygon",
        "protocol": "aave_v2",
        "txHash": f"0x{random.getrandbits(256):x}",
        "logId": f"0x{random.getrandbits(256):x}_Deposit",
        "timestamp": timestamp,
        "blockNumber": random.randint(10000000, 20000000),
        "action": "deposit",
        "actionData": {
            "type": "Deposit",
            "amount": str(round(total_balance, 6)),
            "assetSymbol": "USDC",
            "assetPriceUSD": "1.00",
...
# Fetch data
detailed_wallet_data = [fetch_wallet_data(addr) for addr in wallet_addresses]
detailed_wallet_data = [entry for entry in detailed_wallet_data if entry is not None]

# Save to file
os.makedirs("data", exist_ok=True)
with open("data/detailed_wallet_data.json", "w") as f:
    json.dump(detailed_wallet_data, f, indent=2)

print("✅ Detailed wallet data saved as JSON")
